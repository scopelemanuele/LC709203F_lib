#ifndef LC709203F_h
#define LC709203F_h
#if defined(ARDUINO) && ARDUINO >= 100
#include <Arduino.h>
#else
#include <WProgram.h>
#endif
#include <Wire.h>

#define LC709203F_ADDRESS 0x0B

#define LC709203F_INIT_RSOC 0x07
#define LC709203F_REG_TEMP 0x08
#define LC709203F_REG_CELL_VOLT 0x09
#define LC709203F_REG_APA 0x0b
#define LC709203F_REG_RSOC 0x0d
#define LC709203F_REG_BAT_PROF 0x12
#define LC709203F_REG_POWER 0x15
#define LC709203F_REG_TEMP_METH 0x16
#define LC709203F_REG_CURR_DIR 0X0A
#define LC709203F_REG_ITE 0x0F

const byte CRC_TABLE[256] = {
    };

//  ________________________________________________________
// |Design Capacity of Battery APA(0x0B)                    |
// |--------------------------------------------------------|
// |        |  Type−01,Type−03  |     Type−06 |   Type−07   |
// |--------------------------------------------------------|
// |100 mAh |       0x08        |   0x0D     |   0x07       |
// |200 mAh |       0x0B        |   0x15     |   0x0C       |
// |500 mAh |       0x10        |   0x20     |   0x18       |
// |1000 mAh|       0x19        |   −        |   0x28       |
// |2000 mAh|       0x2D        |   −        |   0x40       |
// |3000 mAh|       0x36        |   −        |   0x4D       |
// ----------------------------------------------------------
//  ________________________________________
// |Design Capacity of Battery APA(0x0B)    |
// |----------------------------------------|
// |        |   Type−04         |   Type−05 |
// |2600 mAh|   0x1A            |   0x0D    |
// ------------------------------------------

class LC709203F {
public:
  // constructor
  LC709203F();
  float getBatteryVoltage();
  uint16_t getBatteryLevel();
  float getBatteryLevelAccu();
  void begin();
  void sleep();
  void wakeup();
  void setProfile(uint16_t profile);
  void setApa(uint16_t apa);

private:
  byte crc8ccitt(const void *data, size_t size);
  uint16_t readReg16(uint8_t deviceAddress, uint8_t regAddress);
  void writeReg16(uint8_t deviceAddress, uint8_t regAddress, uint16_t data);
  uint8_t deviceAddress;
};
#endif
